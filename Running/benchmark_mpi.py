...
# MPI Weak scaling
MPI_WEAK_SCALES = MPI_STRONG_SCALES
...
def start_weak_scaling_benchmark_mpi(iteration):
    ...
    for combination in combinations:
        print("RUNNING FOR " + str(combination))
        combined = combination["simulated_neuron"] + "," + str(combination["nodes"]) + "," + str(combination["network_scale"])
        log(f"\033[93m{combined}\033[0m" if DEBUG else combined)

        filename = "sbatch_run_" + combination["simulated_neuron"] + "_" + str(combination["nodes"]) + "_" + str(combination["network_scale"]) + "_[iter=" + str(iteration) + "].sh"
        filename = os.path.join(dirname, "sbatch", filename)
        # Create the sbatch file
        render_sbatch_template(combination, filename)
        command = ["sbatch", f"{filename}"]
...
def plot_weak_scaling_mpi(data):
    plt.figure()
    neurons = []
    ...
    for neuron, values in data.items():
        ...
        plt.errorbar(x, y, yerr=y_std, label=legend[neuron], color=palette(colors[neuron]), linestyle='-', marker='o', markersize=4, ecolor='gray', capsize=2)

    plt.xlabel('Number of nodes')
    plt.ylabel('Wall clock time (s)')
    plt.xscale('log')
    # plt.yscale('log')
    plt.xticks(MPI_WEAK_SCALES, MPI_WEAK_SCALES)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'weak_scaling_mpi_abs.png'))
    ...
    for neuron, values in data.items():
        ...
        x = np.array([int(val) for val in x], dtype=int)
        plt.errorbar(x, y_factor, yerr=y_factor_std, label=legend[neuron], color=palette(colors[neuron]), linestyle='-', marker='o', markersize=4, ecolor='gray', capsize=2)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'weak_scaling_mpi_rel.png'))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'weak_scaling_mpi.png'))
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(os.path.join(output_folder, STRONGSCALINGMPIFOLDERNAME)):
        if filename.endswith(".json"):
            ...
            with open(os.path.join(output_folder, STRONGSCALINGMPIFOLDERNAME, filename), "r") as f:
                ...
                strong_scaling_data.setdefault(simulated_neuron, {}).setdefault(nodes, {}).setdefault(iteration, json_data)
                f.close()
    plot_strong_scaling_mpi(strong_scaling_data)
    ...
    for filename in os.listdir(os.path.join(output_folder, WEAKSCALINGMPIFOLDERNAME)):
        if filename.endswith(".json"):
            ...
            nodes = int(extract_value_from_filename(filename, "nodes"))
            iteration = int(extract_value_from_filename(filename, "iteration"))
            with open(os.path.join(output_folder, WEAKSCALINGMPIFOLDERNAME, filename), "r") as f:
                data = f.read()
                json_data = clean_json_content(data)
                weak_scaling_data.setdefault(simulated_neuron, {}).setdefault(nodes, {}).setdefault(iteration, json_data)
                f.close()
    plot_weak_scaling_mpi(weak_scaling_data)
