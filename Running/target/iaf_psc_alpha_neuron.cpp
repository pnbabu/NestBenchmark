...
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<iaf_psc_alpha_neuron> iaf_psc_alpha_neuron::recordablesMap_;
namespace nest
{
...
template <> void RecordablesMap<iaf_psc_alpha_neuron>::create()
  {
   ...
   insert_(iaf_psc_alpha_neuron_names::_V_m, &iaf_psc_alpha_neuron::get_V_m);
   insert_(iaf_psc_alpha_neuron_names::_refr_t, &iaf_psc_alpha_neuron::get_refr_t);
   insert_(iaf_psc_alpha_neuron_names::_I_kernel_exc__X__exc_spikes, &iaf_psc_alpha_neuron::get_I_kernel_exc__X__exc_spikes);
   insert_(iaf_psc_alpha_neuron_names::_I_kernel_exc__X__exc_spikes__d, &iaf_psc_alpha_neuron::get_I_kernel_exc__X__exc_spikes__d);
   insert_(iaf_psc_alpha_neuron_names::_I_kernel_inh__X__inh_spikes, &iaf_psc_alpha_neuron::get_I_kernel_inh__X__inh_spikes);
   insert_(iaf_psc_alpha_neuron_names::_I_kernel_inh__X__inh_spikes__d, &iaf_psc_alpha_neuron::get_I_kernel_inh__X__inh_spikes__d);

    // Add vector variables  
  }
}
std::vector< std::tuple< int, int > > iaf_psc_alpha_neuron::rport_to_nestml_buffer_idx =
{
  
  { iaf_psc_alpha_neuron::EXC_SPIKES, iaf_psc_alpha_neuron::INH_SPIKES },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of iaf_psc_alpha_neuron's constructor.
// ---------------------------------------------------------------------------
...
iaf_psc_alpha_neuron::Buffers_::Buffers_(const Buffers_ &, iaf_psc_alpha_neuron &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
  , spike_input_received_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
...
iaf_psc_alpha_neuron::iaf_psc_alpha_neuron():ArchivingNode(), P_(), S_(), B_(*this)
{
...
iaf_psc_alpha_neuron::iaf_psc_alpha_neuron(const iaf_psc_alpha_neuron& __n):
...
// ---------------------------------------------------------------------------
void iaf_psc_alpha_neuron::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void iaf_psc_alpha_neuron::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_neuron::init_state_internal_()" << std::endl;
#endif
...
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_neuron::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void iaf_psc_alpha_neuron::recompute_internal_variables(bool exclude_timestep)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
...
void iaf_psc_alpha_neuron::pre_run_hook()
{
  B_.logger_.init();
...
void iaf_psc_alpha_neuron::update(nest::Time const & origin,const long from, const long to)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function

#ifdef DEBUG
  std::cout << "In iaf_psc_alpha_neuron::update: t = " << __resolution << std::endl;
#endif
...
void iaf_psc_alpha_neuron::handle(nest::SpikeEvent &e)
{
  ...
  else
  {
    ...
    if ( nestml_buffer_idx == iaf_psc_alpha_neuron::PORT_NOT_AVAILABLE )
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
