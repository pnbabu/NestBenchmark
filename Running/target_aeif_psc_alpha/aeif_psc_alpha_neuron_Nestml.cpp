...
#include "aeif_psc_alpha_neuron_Nestml.h"

// ---------------------------------------------------------------------------
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<aeif_psc_alpha_neuron_Nestml> aeif_psc_alpha_neuron_Nestml::recordablesMap_;
namespace nest
{

  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
template <> void RecordablesMap<aeif_psc_alpha_neuron_Nestml>::create()
  {
    // add state variables to recordables map
   insert_(aeif_psc_alpha_neuron_Nestml_names::_V_m, &aeif_psc_alpha_neuron_Nestml::get_V_m);
   insert_(aeif_psc_alpha_neuron_Nestml_names::_w, &aeif_psc_alpha_neuron_Nestml::get_w);
   insert_(aeif_psc_alpha_neuron_Nestml_names::_refr_t, &aeif_psc_alpha_neuron_Nestml::get_refr_t);
   insert_(aeif_psc_alpha_neuron_Nestml_names::_g_inh__X__inh_spikes, &aeif_psc_alpha_neuron_Nestml::get_g_inh__X__inh_spikes);
   insert_(aeif_psc_alpha_neuron_Nestml_names::_g_inh__X__inh_spikes__d, &aeif_psc_alpha_neuron_Nestml::get_g_inh__X__inh_spikes__d);
   insert_(aeif_psc_alpha_neuron_Nestml_names::_g_exc__X__exc_spikes, &aeif_psc_alpha_neuron_Nestml::get_g_exc__X__exc_spikes);
   insert_(aeif_psc_alpha_neuron_Nestml_names::_g_exc__X__exc_spikes__d, &aeif_psc_alpha_neuron_Nestml::get_g_exc__X__exc_spikes__d);

    // Add vector variables  
  }
}
std::vector< std::tuple< int, int > > aeif_psc_alpha_neuron_Nestml::rport_to_nestml_buffer_idx =
{
  
  { aeif_psc_alpha_neuron_Nestml::EXC_SPIKES, aeif_psc_alpha_neuron_Nestml::INH_SPIKES },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of aeif_psc_alpha_neuron_Nestml's constructor.
// ---------------------------------------------------------------------------

aeif_psc_alpha_neuron_Nestml::Parameters_::Parameters_()
{
}

aeif_psc_alpha_neuron_Nestml::State_::State_()
{
}
...
aeif_psc_alpha_neuron_Nestml::Buffers_::Buffers_(aeif_psc_alpha_neuron_Nestml &n):
  logger_(n)
...
aeif_psc_alpha_neuron_Nestml::Buffers_::Buffers_(const Buffers_ &, aeif_psc_alpha_neuron_Nestml &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
  , spike_input_received_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
...
aeif_psc_alpha_neuron_Nestml::aeif_psc_alpha_neuron_Nestml():ArchivingNode(), P_(), S_(), B_(*this)
{
...
aeif_psc_alpha_neuron_Nestml::aeif_psc_alpha_neuron_Nestml(const aeif_psc_alpha_neuron_Nestml& __n):
...
aeif_psc_alpha_neuron_Nestml::~aeif_psc_alpha_neuron_Nestml()
{
...
// ---------------------------------------------------------------------------
void aeif_psc_alpha_neuron_Nestml::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void aeif_psc_alpha_neuron_Nestml::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "aeif_psc_alpha_neuron_Nestml::init_state_internal_()" << std::endl;
#endif
...
void aeif_psc_alpha_neuron_Nestml::init_buffers_()
{
#ifdef DEBUG
  std::cout << "aeif_psc_alpha_neuron_Nestml::init_buffers_()" << std::endl;
#endif
  // spike input buffers
  ...
  // B_.__sys.function = aeif_psc_alpha_neuron_Nestml_dynamics; // will be set just prior to the call to gsl_odeiv_evolve_apply()
...
void aeif_psc_alpha_neuron_Nestml::recompute_internal_variables(bool exclude_timestep)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
...
void aeif_psc_alpha_neuron_Nestml::pre_run_hook()
{
  B_.logger_.init();
...
extern "C" inline int aeif_psc_alpha_neuron_Nestml_dynamics(double __time, const double ode_state[], double f[], void* pnode)
{
  typedef aeif_psc_alpha_neuron_Nestml::State_ State_;
  // get access to node so we can almost work as in a member function
  assert( pnode );
  const aeif_psc_alpha_neuron_Nestml& node = *( reinterpret_cast< aeif_psc_alpha_neuron_Nestml* >( pnode ) );
...
void aeif_psc_alpha_neuron_Nestml::update(nest::Time const & origin,const long from, const long to)
{
...
#ifdef DEBUG
  std::cout << "In aeif_psc_alpha_neuron_Nestml::update: t = " << __resolution << std::endl;
#endif
...
const double g_exc__X__exc_spikes__d__tmp_ = V_.__P__g_exc__X__exc_spikes__d__g_exc__X__exc_spikes * S_.ode_state[State_::g_exc__X__exc_spikes] + V_.__P__g_exc__X__exc_spikes__d__g_exc__X__exc_spikes__d * S_.ode_state[State_::g_exc__X__exc_spikes__d];
    ...
    **/
    ...
    {  
        ...
        double __t = 0;
        B_.__sys.function = aeif_psc_alpha_neuron_Nestml_dynamics;
...
void aeif_psc_alpha_neuron_Nestml::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}


void aeif_psc_alpha_neuron_Nestml::handle(nest::SpikeEvent &e)
{
  ...
  else
  {
    ...
    if ( nestml_buffer_idx == aeif_psc_alpha_neuron_Nestml::PORT_NOT_AVAILABLE )
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
...
void aeif_psc_alpha_neuron_Nestml::handle(nest::CurrentEvent& e)
{
  assert(e.get_delay_steps() > 0);
