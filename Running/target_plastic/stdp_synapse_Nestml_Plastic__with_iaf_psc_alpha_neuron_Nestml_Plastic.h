 ...
 *
 *  Generated from NESTML at time: 2024-03-04 13:36:40.061198
 **/
...
namespace nest
{
  ...
  public:
    ...
    static constexpr ConnectionModelProperties properties = ConnectionModelProperties::HAS_DELAY | ConnectionModelProperties::IS_PRIMARY | ConnectionModelProperties::SUPPORTS_HPC | ConnectionModelProperties::SUPPORTS_LBL;
...

    {
      ...
      double timestep = 0;
      {
        ...
        while (start != finish)
        {
          ...
          auto get_t = [_tr_t]()
          { return _tr_t; }; // do not remove, this is in case the predefined time variable ``t`` is used in the NESTML model
          /**
      ...
      {
        auto get_t = [__t_spike]()
        { return __t_spike; }; // do not remove, this is in case the predefined time variable ``t`` is used in the NESTML model
                               /**
  ...
  stdp_synapse_Nestml_Plastic__with_iaf_psc_alpha_neuron_Nestml_Plastic<targetidentifierT>::update_internal_state_(double t_start, double timestep, const stdp_synapse_Nestml_Plastic__with_iaf_psc_alpha_neuron_Nestml_PlasticCommonSynapseProperties &cp)
  {
    ...
    auto get_t = [t_start]()
    { return t_start; }; // do not remove, this is in case the predefined time variable ``t`` is used in the NESTML model
