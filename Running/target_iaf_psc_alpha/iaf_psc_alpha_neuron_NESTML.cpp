...
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<iaf_psc_alpha_neuron_NESTML> iaf_psc_alpha_neuron_NESTML::recordablesMap_;
namespace nest
{
  ...
  template <>
  void RecordablesMap<iaf_psc_alpha_neuron_NESTML>::create()
  {
    // add state variables to recordables map
    insert_(iaf_psc_alpha_neuron_NESTML_names::_V_m, &iaf_psc_alpha_neuron_NESTML::get_V_m);
    insert_(iaf_psc_alpha_neuron_NESTML_names::_refr_t, &iaf_psc_alpha_neuron_NESTML::get_refr_t);
    insert_(iaf_psc_alpha_neuron_NESTML_names::_I_kernel_inh__X__inh_spikes, &iaf_psc_alpha_neuron_NESTML::get_I_kernel_inh__X__inh_spikes);
    insert_(iaf_psc_alpha_neuron_NESTML_names::_I_kernel_inh__X__inh_spikes__d, &iaf_psc_alpha_neuron_NESTML::get_I_kernel_inh__X__inh_spikes__d);
    insert_(iaf_psc_alpha_neuron_NESTML_names::_I_kernel_exc__X__exc_spikes, &iaf_psc_alpha_neuron_NESTML::get_I_kernel_exc__X__exc_spikes);
    insert_(iaf_psc_alpha_neuron_NESTML_names::_I_kernel_exc__X__exc_spikes__d, &iaf_psc_alpha_neuron_NESTML::get_I_kernel_exc__X__exc_spikes__d);

    // Add vector variables
  }
}
std::vector<std::tuple<int, int>> iaf_psc_alpha_neuron_NESTML::rport_to_nestml_buffer_idx =
    {

        {iaf_psc_alpha_neuron_NESTML::EXC_SPIKES, iaf_psc_alpha_neuron_NESTML::INH_SPIKES},
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of iaf_psc_alpha_neuron_NESTML's constructor.
// ---------------------------------------------------------------------------

iaf_psc_alpha_neuron_NESTML::Parameters_::Parameters_()
{
}

iaf_psc_alpha_neuron_NESTML::State_::State_()
{
}
...
iaf_psc_alpha_neuron_NESTML::iaf_psc_alpha_neuron_NESTML() : ArchivingNode(), P_(), S_(), B_(*this)
{
...
iaf_psc_alpha_neuron_NESTML::iaf_psc_alpha_neuron_NESTML(const iaf_psc_alpha_neuron_NESTML &__n) : ArchivingNode(), P_(__n.P_), S_(__n.S_), B_(__n.B_, *this)
{
...
iaf_psc_alpha_neuron_NESTML::~iaf_psc_alpha_neuron_NESTML()
{
}
...
// ---------------------------------------------------------------------------
void iaf_psc_alpha_neuron_NESTML::calibrate_time(const nest::TimeConverter &tc)
{
  LOG(nest::M_WARNING,
...
void iaf_psc_alpha_neuron_NESTML::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_neuron_NESTML::init_state_internal_()" << std::endl;
#endif
...
void iaf_psc_alpha_neuron_NESTML::init_buffers_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_neuron_NESTML::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void iaf_psc_alpha_neuron_NESTML::recompute_internal_variables(bool exclude_timestep)
{
  const double __resolution = nest::Time::get_resolution().get_ms(); // do not remove, this is necessary for the resolution() function
...
void iaf_psc_alpha_neuron_NESTML::pre_run_hook()
{
  B_.logger_.init();
...
#ifdef DEBUG
  std::cout << "In iaf_psc_alpha_neuron_NESTML::update: t = " << __resolution << std::endl;
#endif
...
void iaf_psc_alpha_neuron_NESTML::handle(nest::DataLoggingRequest &e)
{
  B_.logger_.handle(e);
}

void iaf_psc_alpha_neuron_NESTML::handle(nest::SpikeEvent &e)
{
  ...
  else
  {
    ...
    if (nestml_buffer_idx == iaf_psc_alpha_neuron_NESTML::PORT_NOT_AVAILABLE)
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
...
void iaf_psc_alpha_neuron_NESTML::handle(nest::CurrentEvent &e)
{
  assert(e.get_delay_steps() > 0);
