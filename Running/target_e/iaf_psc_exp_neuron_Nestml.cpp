...
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<iaf_psc_exp_neuron_Nestml> iaf_psc_exp_neuron_Nestml::recordablesMap_;
namespace nest
{

  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
template <> void RecordablesMap<iaf_psc_exp_neuron_Nestml>::create()
  {
    // add state variables to recordables map
   insert_(iaf_psc_exp_neuron_Nestml_names::_V_m, &iaf_psc_exp_neuron_Nestml::get_V_m);
   insert_(iaf_psc_exp_neuron_Nestml_names::_refr_t, &iaf_psc_exp_neuron_Nestml::get_refr_t);
   insert_(iaf_psc_exp_neuron_Nestml_names::_I_syn_exc, &iaf_psc_exp_neuron_Nestml::get_I_syn_exc);
   insert_(iaf_psc_exp_neuron_Nestml_names::_I_syn_inh, &iaf_psc_exp_neuron_Nestml::get_I_syn_inh);

    // Add vector variables  
  }
}
std::vector< std::tuple< int, int > > iaf_psc_exp_neuron_Nestml::rport_to_nestml_buffer_idx =
{
  
  { iaf_psc_exp_neuron_Nestml::EXC_SPIKES, iaf_psc_exp_neuron_Nestml::INH_SPIKES },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of iaf_psc_exp_neuron_Nestml's constructor.
// ---------------------------------------------------------------------------

iaf_psc_exp_neuron_Nestml::Parameters_::Parameters_()
{
}

iaf_psc_exp_neuron_Nestml::State_::State_()
{
}
...
iaf_psc_exp_neuron_Nestml::Buffers_::Buffers_(iaf_psc_exp_neuron_Nestml &n):
  logger_(n)
...
iaf_psc_exp_neuron_Nestml::Buffers_::Buffers_(const Buffers_ &, iaf_psc_exp_neuron_Nestml &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
  , spike_input_received_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
...
iaf_psc_exp_neuron_Nestml::iaf_psc_exp_neuron_Nestml():ArchivingNode(), P_(), S_(), B_(*this)
{
...
iaf_psc_exp_neuron_Nestml::iaf_psc_exp_neuron_Nestml(const iaf_psc_exp_neuron_Nestml& __n):
...
// ---------------------------------------------------------------------------
void iaf_psc_exp_neuron_Nestml::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void iaf_psc_exp_neuron_Nestml::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_exp_neuron_Nestml::init_state_internal_()" << std::endl;
#endif
...
void iaf_psc_exp_neuron_Nestml::init_buffers_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_exp_neuron_Nestml::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void iaf_psc_exp_neuron_Nestml::recompute_internal_variables(bool exclude_timestep)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
...
void iaf_psc_exp_neuron_Nestml::pre_run_hook()
{
  B_.logger_.init();
...
void iaf_psc_exp_neuron_Nestml::update(nest::Time const & origin,const long from, const long to)
{
...
#ifdef DEBUG
  std::cout << "In iaf_psc_exp_neuron_Nestml::update: t = " << __resolution << std::endl;
#endif
...
void iaf_psc_exp_neuron_Nestml::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}


void iaf_psc_exp_neuron_Nestml::handle(nest::SpikeEvent &e)
{
  ...
  else
  {
    ...
    if ( nestml_buffer_idx == iaf_psc_exp_neuron_Nestml::PORT_NOT_AVAILABLE )
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
...
void iaf_psc_exp_neuron_Nestml::handle(nest::CurrentEvent& e)
{
  assert(e.get_delay_steps() > 0);
...
// -------------------------------------------------------------------------
void
iaf_psc_exp_neuron_Nestml::on_receive_block_exc_spikes()
{  
...
void
iaf_psc_exp_neuron_Nestml::on_receive_block_inh_spikes()
{  
